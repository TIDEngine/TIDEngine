...
class Mysql_Wrapper
{
	...
	public function connect($sConfigLabel='default')
	{
		...
		if($aParams 	!= null)
		{
			$this->oCon	= mysql_connect($aParams['SERVER'], $aParams['USER'], $aParams['PASS']);
			if(!$this->oCon)
			{
				$this->showMessage(str_replace('[##SERVER_NAME##]',$aParams['SERVER'],$this->aErrorMessages['SERVER_NOT_FOUND']));
			}
			elseif(!mysql_select_db($aParams['DATABASE'], $this->oCon))
			{
	...
	{
		if($this->oCon!=false && $this->oCon!=null && !mysql_close($this->oCon))
		{
	...
	{
		return mysql_real_escape_string($sString, $this->oCon);
	}
	...
	{
		$this->oQuery = mysql_query($sQuery,$this->oCon) or $this->showMessage(mysql_error($this->oCon));
		$this->iAffectedRows = mysql_affected_rows($this->oCon);
		return $this->oQuery;
	}
	...
	public function executeInsert($sQuery)
	{
		...
		$this->iLastInsertedID = mysql_insert_id($this->oCon);
	...
	public function fetch($oQuery=false)
	{
		...
		if($this->oQuery!=null)
		{
			$aRow		= mysql_fetch_assoc($this->oQuery);
		}
		else{

...
elseif(isset($_REQUEST['ax-file-name']))
{
	...
	if($fullPath)
	{
	    ...
	    if($isLast=='true')
	    {
	    	$thumb = createThumbGD($fullPath, $thumbPath, $thumbPostfix, $thumbWidth, $thumbHeight, $thumbFormat);
	    }
	    echo json_encode(array('name'=>array('file'=>basename($fullPath), 'thumb'=>$thumb), 'size'=>$currByte, 'status'=>'uploaded', 'info'=>'File/chunk uploaded'));
	}
}